      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="text-left text-[11px] text-muted border-b border-default bg-muted">
            ...
            <th class="px-3 py-1.5 font-medium">Environment</th>
            <th class="px-3 py-1.5 font-medium text-right" :class="by === 'requests' ? 'text-highlighted' : ''">Requests</th>
            <th class="px-3 py-1.5 font-medium text-right" :class="by === 'errorRate' ? 'text-highlighted' : ''">Errors</th>
            <th class="px-3 py-1.5 font-medium text-right" :class="by === 'p95' ? 'text-highlighted' : ''">p95</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="!entries?.length">
            <td colspan="5" class="px-3 py-4 text-center text-xs text-muted">{{ loading ? "Loading…" : empty }}</td>
          </tr>
          <tr v-for="entry in entries ?? []" :key="entry.key" class="border-b border-muted last:border-0">
            <td class="px-3 py-1.5 font-mono text-xs text-highlighted break-all">{{ entry.key || "—" }}</td>
            ...
            <td class="px-3 py-1.5 text-right font-mono text-xs tabular-nums text-toned">
              {{ compactCount(entry.requests) }}
              <span class="text-dimmed">· {{ formatRate(entry.requestsPerSecond) }}</span>
            </td>
            <td
              class="px-3 py-1.5 text-right font-mono text-xs tabular-nums"
