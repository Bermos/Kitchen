...
import { api, type PlatformRetention, type PlatformRetentionPatch, type RetentionClass } from "../lib/api";
...
const { data, error, loading, refresh } = useAsync(() => api.platformRetention());

const caller = computed(() => callerFor());
const mayWrite = computed(() => may("PATCH /api/v1/platform/retention", caller.value));
...
const moved = computed(() => {
  ...
  return value.classes.filter((entry) => days.value[entry.class] !== entry.days).map((entry) => entry.class);
});
...
const needsOverride = computed(() => {
  ...
  return audit !== undefined && audit < value.auditFloorDays && !value.auditFloorOverride;
});
...
async function save() {
  ...
  if (needsOverride.value) {
    body.auditFloorOverride = { reason: overrideReason.value, approvedBy: overrideApprovedBy.value };
  }

  saving.value = true;
  try {
    await api.updatePlatformRetention(body);
    toast.add({ title: "Retention saved", color: "success", icon: "i-lucide-check" });
    overrideReason.value = "";
    overrideApprovedBy.value = "";
    await refresh();
  } catch (err) {
    // The floor's refusal is a sentence worth reading in full — it names the
    // field, the number and the way past it — so it is the description rather
    // than a title trimmed to fit.
    toast.add({
...
function oldestOf(entry: RetentionClass): string {
  return entry.oldest ? new Date(entry.oldest).toLocaleDateString() : "—";
}

const sweptAt = computed(() => (data.value?.lastSweep ? new Date(data.value.lastSweep).toLocaleString() : ""));
</script>
  ...
  <div class="rounded-md border border-default px-5 py-4 space-y-4">
    <div>
      ...
      <p class="text-xs text-muted mt-1">
        How long each class of what the platform keeps is kept, and how far back each one actually goes. The
        <span class="font-mono">oldest</span> column is the claim retention makes: nothing of that class is older than
        this.<template v-if="sweptAt"> Last measured {{ sweptAt }}.</template>
      </p>
    </div>

    <UAlert v-if="error" color="error" variant="soft" icon="i-lucide-triangle-alert" :title="error" />
    <UAlert v-else-if="data?.message" color="warning" variant="soft" icon="i-lucide-info" :description="data.message" />
    ...
    <template v-if="data">
      <div class="rounded-md border border-default bg-muted overflow-x-auto">
        <table class="w-full min-w-[42rem] text-sm">
          ...
          <tbody>
            <tr v-for="entry in data.classes" :key="entry.class" class="border-b border-muted last:border-0">
              <td class="px-3 py-2">
                <span class="inline-flex items-center gap-2">
                  <StatusDot :tone="entry.enforced ? 'success' : 'neutral'" />
                  <span>
                    <span class="text-highlighted">{{ entry.label }}</span>
                    <span class="block text-xs text-muted">{{ entry.description }}</span>
                  </span>
                </span>
              </td>
              <td class="px-3 py-2">
                <UInputNumber v-model="days[entry.class]" :min="1" :disabled="!mayWrite" class="w-28" />
              </td>
              <td class="px-3 py-2 font-mono text-xs text-toned">
                {{ entry.source === "retention" ? "set here" : entry.source }}
              </td>
              <td class="px-3 py-2 font-mono text-xs" :class="entry.oldest ? 'text-toned' : 'text-muted'">
                {{ oldestOf(entry) }}
              </td>
              <td class="px-3 py-2 font-mono text-xs text-toned">{{ entry.rows ?? "—" }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div v-if="needsOverride" class="rounded-md border border-warning/40 bg-warning/5 px-4 py-3 space-y-3">
        <p class="text-xs text-toned">
          Audit records are the evidence an incident is reconstructed from, and an incident reporting duty runs from
          when an institution became aware — which can be long after the transition that caused it. Keeping less than
          {{ data.auditFloorDays }} days needs a written override, and using it is itself an audit record.
        </p>
        <UFormField label="Why" help="Read by whoever asks why the log does not go back far enough.">
          <UTextarea v-model="overrideReason" :rows="2" class="w-full" />
        </UFormField>
        <UFormField label="Approved by" help="Whoever decided it, as a name or an address.">
      ...
      <div class="flex items-center justify-end gap-3">
        ...
        <UButton :disabled="!mayWrite || !moved.length" :loading="saving" icon="i-lucide-save" @click="save">
          Save retention
        </UButton>
      </div>
    </template>
    <div v-else-if="loading" class="py-8 text-center text-muted text-sm">Loading…</div>
  </div>
</template>
