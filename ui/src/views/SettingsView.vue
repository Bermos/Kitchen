  ...
  <div class="space-y-6 max-w-3xl">
    <div>
      ...
      <p class="text-xs text-muted mt-1">
        The platform's runtime configuration — the
        <span class="font-mono">Kitchen</span> singleton the operator
        reconciles.
      </p>
    </div>
    ...
    <template v-else-if="settings">
      <div
        class="rounded-md border border-default bg-muted px-5 py-4 grid gap-x-8 gap-y-4 sm:grid-cols-2 text-sm"
      >
        <div>
        ...
        <div class="min-w-0">
          ...
          <p
            class="font-mono text-toned truncate"
            :title="settings.apiExternalURL"
          >
        ...
        <div>
          ...
          <p class="font-mono text-toned">
            {{ settings.gatewayClassName || "—"
            }}<template v-if="settings.gatewayAddress">
      ...
      <div class="rounded-md border border-default px-5 py-4 space-y-4">
        <h2 class="text-sm font-medium text-highlighted">
          Builds and telemetry
        </h2>
        <UFormField
          label="Default build strategy"
          help="Projects can override this per repository."
        >
          <USelect
            v-model="strategy"
            :items="strategies"
        ...
        <UFormField
          ...
          help="How many builds run at once, platform-wide."
        >
      ...
      <div v-if="operatorMode">
        ...
        <p class="text-xs text-muted mb-3">
          Every workload labelled as part of Kitchen, and whether the pods it
          wants are running. A component with no pods at all was refused before
          it started — the message carries the reason.
        </p>
          ...
          <table class="w-full min-w-[36rem] text-sm">
            ...
            <tbody>
              <tr v-if="!components.length">
                <td colspan="4" class="px-3 py-3 text-muted">
                  No components surveyed yet.
                </td>
              </tr>
              <tr
              ...
              >
                <td class="px-3 py-2 font-mono text-highlighted">
                  <span class="inline-flex items-center gap-2">
                    <StatusDot
                      :tone="component.healthy ? 'success' : 'error'"
                    />
                    {{ component.name }}
                  </span>
                </td>
                ...
                <td
                  class="px-3 py-2 text-xs text-toned max-w-md truncate"
    ...
    <div v-else-if="loading" class="py-24 text-center text-muted text-sm">
      Loading…
    </div>
  </div>
</template>
