...
export interface BackupDestination {
  ...
  serverSideEncryption?: string;
  kmsKeyId?: string;
  /** An endpoint that is not https, admitted on purpose. */
  allowInsecureEndpoint?: boolean;
  credential: "stored" | "ambient";
}

/**
 * What protects an archive at the destination.
 *
 * `mode` is `aes256-gcm` — the archive encrypted under this platform's own key
 * before it is uploaded — or `none`, which an installation has to ask for out
 * loud. `key` says only whether there is one, never what it is: an
 * installation encrypting its archives with no key stored is one whose next
 * run will refuse to upload rather than write every credential the platform
 * holds into somebody's bucket in the clear.
 */
export interface BackupEncryption {
  mode: "aes256-gcm" | "none";
  key: "stored" | "absent";
}
...
export interface BackupDestinationWrite {
  ...
  /** The archive's own encryption. `key` is 32 bytes as base64 or hex, is
   * supplied rather than generated by the platform, and is never read back —
   * so leaving it out means "leave the stored key alone". A first destination
   * carries one or says `mode: "none"`, because the route refuses to write a
   * destination whose archives are neither encrypted nor deliberately not. */
  encryption?: {
    mode?: "aes256-gcm" | "none";
    key?: string;
  };
