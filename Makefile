...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	# The Dockerfile pins its build stages to $$BUILDPLATFORM itself, so this
	# builds it directly. It used to be copied to a Dockerfile.cross with that
	# flag patched into the first FROM by sed, which reached only one of the two
