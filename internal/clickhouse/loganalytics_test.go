...
func TestTheHistogramBucketsTheWindow(t *testing.T) {
	...
	if !strings.Contains(store.query, "`level` = {q0:String}") {
		t.Fatalf("the query should compile into the histogram's predicate:\n%s", store.query)
	}
	if got := store.params.Get("param_q0"); got != "error" {
...
func TestFacetsAreCountedOverTheWindow(t *testing.T) {
	...
	if facets[0].Values[0].Value != "error" || facets[0].Values[0].Count != 120 {
...
// A facet on something that is not a column is a structured field, resolved the
// same way the query language resolves it — so a user can facet on what they
// can query on.
func TestAFacetOnAStructuredFieldResolvesLikeAQuery(t *testing.T) {
	store := newFakeLogStore(t)
	...
	if !strings.Contains(store.query, "fields[{facet0:String}]") {
		t.Fatalf("a facet on a field should read the map:\n%s", store.query)
	}
