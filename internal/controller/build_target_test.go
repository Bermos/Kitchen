	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			build := &kitchenv1alpha1.Build{
				Status: kitchenv1alpha1.BuildStatus{DockerfileTarget: tc.target},
			}
			if got := targetNotFound(build, tc.failure); got != tc.want {
...
// The message says the three things BuildKit's own sentence leaves out: which
// file was looked in, which stage was asked for, and which of the two places a
...
func TestTheSentenceAStageThatIsNotThereGets(t *testing.T) {
	...
	message := targetNotFoundMessage(project, build)
	...
	if message := targetNotFoundMessage(project, build); !strings.Contains(message, "kitchen.json") {
