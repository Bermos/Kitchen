...
var _ = Describe("Project secrets", func() {
	...
	// The digest that makes a rotation reach a running pod. It covers only
	// what the workload reads, so an unrelated secret does not roll every
	// environment the project has.
	Describe("the revision a workload is stamped with", func() {
		reads := func(key string) []corev1.EnvVar {
			return []corev1.EnvVar{{
				Name: "DB_PASS",
				ValueFrom: &corev1.EnvVarSource{SecretKeyRef: &corev1.SecretKeySelector{
					LocalObjectReference: corev1.LocalObjectReference{Name: ProjectSecretsName},
					Key:                  key,
				}},
			}}
		}

		It("is empty for a workload that reads no project secret", func() {
			write(map[string][]byte{"SMTP_PASSWORD": []byte("first")})
			reconcileOnce()

			revision, err := projectSecretsRevision(ctx, k8sClient, appNS,
		...
		It("changes when the value it reads changes, and not otherwise", func() {
			write(map[string][]byte{"SMTP_PASSWORD": []byte("first"), "UNRELATED": []byte("a")})
			reconcileOnce()

			before, err := projectSecretsRevision(ctx, k8sClient, appNS, reads("SMTP_PASSWORD"))
			Expect(err).NotTo(HaveOccurred())
			Expect(before).NotTo(BeEmpty())

			By("changing a secret this workload does not read")
			write(map[string][]byte{"SMTP_PASSWORD": []byte("first"), "UNRELATED": []byte("b")})
			reconcileOnce()
			unmoved, err := projectSecretsRevision(ctx, k8sClient, appNS, reads("SMTP_PASSWORD"))
			Expect(err).NotTo(HaveOccurred())
			Expect(unmoved).To(Equal(before))

			By("rotating the one it does")
			write(map[string][]byte{"SMTP_PASSWORD": []byte("rotated"), "UNRELATED": []byte("b")})
			reconcileOnce()
			after, err := projectSecretsRevision(ctx, k8sClient, appNS, reads("SMTP_PASSWORD"))
			Expect(err).NotTo(HaveOccurred())
			Expect(after).NotTo(Equal(before))
		})

		It("comes off a pod template that stops reading one", func() {
			...
			applyProjectSecretsRevision(template, "")
			...
			applyProjectSecretsRevision(template, "0123456789abcdef")
			Expect(template.Annotations).To(HaveKeyWithValue(ProjectSecretsRevisionAnnotation, "0123456789abcdef"))
		})
	})
})
