...
func TestProcessPodSpecProbes(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			spec := processPodSpec("env", release, project, nil, nil, tc.process, nil, podInit{})
...
func TestProcessPodSpecCarriesTheProjectsSecurityPosture(t *testing.T) {
	...
	spec := processPodSpec("env", release, project, nil, nil,
		kitchenv1alpha1.ProcessSpec{Name: "worker", Type: kitchenv1alpha1.ProcessWorker}, nil, podInit{})
