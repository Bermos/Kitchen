...
| `publicEndpoint` | none | A second address the same store answers on from outside the cluster, carried into every binding as `publicEndpoint` and signed against by a presigned URL handed to somebody else (#601). The operator writes it on the connection it seeds for the bundled store, which is reached in-cluster at a `.svc` address; a store whose `endpoint` is already public needs none |
