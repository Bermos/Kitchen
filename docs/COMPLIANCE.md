...
for one of the build's artifacts: every attestation attached to that digest,
decoded, and marked verified where a signature was accepted by the platform's
key. `?workload=` says which artifact; without it, the project's own image.
An evidence set gathered with no keys reports itself as a listing rather than
a verification — a reader that could not tell the two apart would eventually
...
### 5.7 A unit is several artifacts, and each carries its own evidence

Since #271 one commit can produce several images: a monorepo ships an API, a
front end and a worker from one repository, and the Release deploys all of
them. The attestation path did not move with it, and the result was the shape
this whole document exists to prevent — **a compliance surface reporting
success over what it never looked at**. A unit of five workloads shipped with
provenance and a bill of materials for the web process and *nothing at all*
for the other four, and nothing said so.

So evidence is per artifact, at every level:

- **Attestation.** Each image is harvested and signed in its own right, about
  its own subject digest. §6.3's rule — the artifact is the image manifest,
  because that is what the builder's statements are about — is applied once per
  image rather than once per commit.
- **Gates.** A quality gate is a claim about an image, so each configured gate
  runs once per image, under a Job of its own, and its result is recorded
  against the image it ran over. A run that failed names that image, because
  "the gate did not run" beside four workloads that did is not an answer.
- **Exploitability.** A VEX statement suppresses a finding on an image, and
  "this CVE does not apply" said about the API is not a claim about the worker
  — which may not carry the package at all. A submission names the image, and
  the same document filed about two images is two assertions.
- **Evaluation.** The policy input carries every artifact's evidence, each
  entry naming its workload, and the "newest vulnerability scan" collapse
  (§9.9) is applied *within* an image rather than across a unit — collapsing
  across would drop the worker's findings for being older than the API's.

#### The Release-level answer

**A release is attested when every artifact it deploys is**, and when it is
not, the answer names the ones that are missing. That is the whole of the
design decision, and it is deliberately not a boolean: a unit that half
carries evidence is a unit somebody has to be able to act on, and "not
attested" without a name is a search rather than an answer.

The two ways to be missing are one answer — an image that pushed and could not
be attested, and an image the Build holds no artifact record for at all —
because from the outside they are the same fact: nothing signed describes an
image this release deploys.

#### Typed by the source of the claim

Each artifact's evidence record carries a **source type**, and it is an enum
with exactly one value today: `built`, meaning the platform built this
artifact and its evidence is the build's own.

One value is not an oversight, it is the point. Vendored software (#309) is
heterogeneous evidence by construction — one workload with full builder
provenance beside one carrying only a vendor's assertion and one with a bill
of materials the platform generated by observation — and a reader has to be
able to tell those apart *without knowing which release of Kitchen wrote the
field*. A distinction that arrives later as a new shape is a distinction every
existing reader silently gets wrong; a distinction that arrives as a new enum
value is one they can refuse to interpret. So the field exists now, says the
one true thing, and gains values rather than gaining a shape.

#### What did not change

A single-workload project's evidence is byte for byte what it was. The
project's own image is attested exactly as before — the same subject, the same
predicate, the same keys in the same order — and its gate Jobs keep the names
they had. The build record signed for a *workload's* image is the one that
grew keys: `workload`, and that workload's own strategy and detected
framework, which a unit does not have one of.

This is not a re-attestation of everything already shipped, and it must not
become one: an artifact is built once and never rebuilt, so re-signing it
under a changed predicate would produce a second claim about the same digest
that disagrees with the first in a field nobody changed on purpose.

#### What is not yet per artifact

The scheduled re-evaluation pass (§9) scans the release's own image. Extending
it to scan every image of a unit is a larger change — one scan Job, one rescan
status and one drift row per artifact rather than per environment — and it is
named here as an open item rather than implied by silence.
...
- **A unit of five workloads runs five times as many gate Jobs.** A gate is a
  claim about an image and there are five images, so there is no cheaper
  honest answer; what there is instead is a bound, since they run after the
  build against the same application namespace as everything else. An
  installation that finds the cost unacceptable turns gates off, which is a
  decision somebody made rather than a coverage gap nobody saw.
...
It is one row per **image**, not one per release (§5.7): a release of a
five-workload unit is five rows, told apart by `workload`, each with its own
digest, its own gates and its own exploitability assertions. A single row per
release would be an index of one image presented as the index of what was
deployed, which is the failure the section exists to make impossible.
...
- **One commit is one build and several artifacts.** `status.artifact` is the
  project's own image and nothing else's; each row of `status.workloads` has
  its own. Anything that answers a question about "the build's evidence" has
  to iterate `Build.Artifacts()`, or it answers about one image and reads as
  if it answered about the release.
- **A release is attested only when every image it deploys is.** The Build's
  `FullyAttested` is the one implementation of that, and
  `ArtifactsWithoutEvidence` names the images that are missing. A surface that
  reported `status.artifact.attestedAt != nil` as the unit's answer would be
  green over four images nothing looked at.
