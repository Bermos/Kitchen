...
| GET | `/projects/{name}/env` | The same variables with the literal values the project typed — the one route that answers a stored value. A variable reading a secret or a claim answers the reference it names, never what is behind it, and the read is recorded where the installation keeps an audit log | `developer` |
...
| PATCH | `/environments/{name}/requirements` | Change the bar it sets — the policy bundle, its parameters, its owners, and who it serves (`serves`: which classes of another project's environments may bind to an offering served from here) | `viewer` to reach; the handler admits only `spec.owners` and operators |
...
| POST | `/connections/{name}/detect` | What the platform makes of a repository, read the way a build would, before the project exists. Only for repositories the connection lists; `403` otherwise | any person |
...
| POST | `/claims/{name}/request` | Ask again for a binding the providing project refused, on the same claim. No body | `developer` |
