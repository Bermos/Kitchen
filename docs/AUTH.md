...
**Platform scope — a machine credential's, and never a person's.**

| Scope | What it reaches |
|---|---|
| `platform.read` | The platform's own read-only surface: nodes, workloads, edge, storage, cluster events, ingest, signals and retention |
| `compliance.read` | The evidence surface: who holds what, the access recertifications, the compliance posture, the audit chain's verification, and a project's audit pack |
| `backup.run` | Taking a platform backup, and reading what one would carry |

A scope is **not a third axis of the role model** — it is what a *platform
credential* holds instead of a role, and it is described in full under
[Platform credentials](#platform-credentials). It is listed here because the
question "may this caller export the platform's state" now has two right
answers: they are an operator, or they hold `backup.run`. Nothing else changed
— a person holds a role and never a scope, and a credential holds scopes and
never a role.
...
| Surface | Role |
|---|---|
| `/platform/*`, `PATCH /settings`, `/connections/{name}` (bar its repository listing and its preflight) and every connection write, `/updates`, `GET /environments/{name}/objects`, `GET /compliance`, `GET /audit/verify` | `operator` — some of these are also reached by a platform credential holding the scope the row names, see [Platform credentials](#platform-credentials) |
...
| Builds and cancellations, releases, environment variables — changing them, and reading what each one's own value is — the project's own secrets, environments, domains, claims | project `developer` |
| Projects — including a variable's name and whether it has a value, never the value — builds, releases, environments, logs, metrics, requests, diagnostics, signals, traces, and a project's members and keys | project `viewer` |
...
**Which values the "never read back" rule covers, and which it does not.** A
credential is what the platform holds *on somebody's behalf*: a connection's
token, a project's own secret, the content of a secret configuration file, a
claim's binding, a password. No route answers one, and no `?privileged=true`
record carries one either. **A literal somebody typed into an environment
variable is not one** (#598) — `spec.env[].value` is cleartext on the Project
object, and declining to answer it cost a developer the ability to check what
`LOG_LEVEL` is set to while protecting nothing `kubectl get project -o yaml`
did not already hand over. So `GET /projects/{name}/env` answers the literals,
and three things hold it in place:

- It is `developer`, the role that may already overwrite a literal, rather
  than the `viewer` every other project read is. A literal can be a URL with a
  token in it, and widening a payload without widening the argument for it is
  how a read comes to carry more than its role was justified against.
- It is a **route of its own**, so `GET /projects/{name}` and `GET
  /projects/{name}/secrets` go on carrying no value at all. That is what keeps
  the secrets list a viewer's read for the reason it always was: it is names
  only.
...
| A platform credential — [the same](#platform-credentials), and this row is the point: it is widened at the *operator API* and by nothing here | Exactly what a CI key reaches. `GET /token` and `/get-session`, and no more of the issuer for reaching more of the platform |
...
**A credential is issued by Kitchen, and never at the issuer.** The api-key plugin
...
stated rather than left to nobody happening to post there: **a credential comes
from the Kitchen API — `POST /projects/{name}/keys` or
`POST /platform/credentials` — and belongs to an account created for it, or it
does not exist.** The platform credential #349 added needed no exception to it,
because it is issued through the platform's own path too.
...
A credential that reaches more than a CI key was expected to be a third row in
that table with different paths in it. #349 landed and the answer turned out to
be that nothing moves: what a platform credential may do is Kitchen's state
rather than the issuer's, so it is a third row saying *the same* paths.

### Platform credentials

A machine account is a member of one project. A **platform credential** is the
other thing a machine can be: an account holding *scopes* on the platform
itself — named operations, chosen when it is issued — so that a scheduled job
or an agent can run the handful of operations an operator would otherwise have
to run by hand ([#349](https://github.com/Bermos/Kitchen/issues/349)).

It is the same shape as a CI key one level up, and deliberately so: a
credential at the issuer, an account created to own it because a key has no
`sub` of its own, and a grant on the object the access is about. What differs
is the object and what the grant says.

| | A project's CI key | A platform credential |
|---|---|---|
| Account's address | `<project>.<key>@machines.kitchen.local` | `<name>@platform.kitchen.local` |
| Grant lives on | the Project's `spec.access` | the Kitchen singleton's `spec.access.credentials` |
| What the grant says | a project **role** | platform **scopes**, and an expiry |
| Issued by | `POST /projects/{name}/keys` | `POST /platform/credentials` |
| Reaches at the issuer | `GET /token`, `/get-session` | exactly the same |

**Why scopes and not a fourth role.** The platform has two roles and a project
has three; "may read the retention policy" is not a hat anybody wears, it is one
operation a cron job needs. A role to hold it would have to be ordered against
`operator`, which is a question with no honest answer — a credential that may
read retention is not *less operator* than one that may take a backup, it is a
different thing. So the two answers compose: `PlatformRoleFor` says what hat the
caller wears, `ScopesFor` says which operations they were additionally handed,
and the guard admits a route when either suffices. An operator needs no scope
and a credential holds no role.

**What a scope may reach is decided by the route table and by nothing else.**
Each row of `internal/api/policy.go` either names a scope or does not, and one
that does not is the operator's forever, however many scopes are added later.
That default-deny is the whole of the blast-radius argument: opening a route to
credentials is an edit to the table, in the same line as the sentence that says
what the route does, reviewed with it. Four categories of route are never
opened, and a test holds it — `TestNoScopeReachesCredentialIssuance`:

- **issuing credentials**, of either kind, so no credential can mint its own
  successor or outlive its own expiry by issuing a fresh one;
- **`PATCH /settings`**, which carries the operator list;
- **`/updates`**, which is upgrading the platform;
- **every connection write**, which carries a credential.

**It expires, and the expiry is enforced where the scope is resolved.**
`Expires` is required on the grant, and `access.ScopesFor` applies it at every
request — so a leaked credential lapses on its own, on every replica, with
nothing having had to run. The API defaults a new one to 30 days and refuses
more than 90: an operator issuing one cannot know which pipeline or laptop it
is about to be pasted into, and a credential that has to outlive a quarter is
one to reissue, which is an action somebody takes and the log records. An entry
with **no** expiry recorded reads as lapsed rather than as eternal, which is the
safe reading of an object written around the API server.

The sweep in the operator (`AccessSweeper`) then removes a lapsed entry and the
account behind it. That is hygiene rather than enforcement — the credential
already holds nothing — which is why it runs outside the compliance gate and
does nothing quietly when the issuer cannot be reached.

**Reading and writing are separable, and that is what the scopes are split
on.** `backup.run` is its own scope because a platform backup is every custom
resource, every Secret in `kitchen-system` and the identity provider's database
in the clear: a scope that lumped it in with the reads would turn the cron job
that watches retention into a credential that can exfiltrate the installation.
Reading *what a backup would carry* takes the same scope, because it enumerates
which secrets that is.

**A credential can be narrowed to named projects.** One scoped route is about a
project rather than about the platform — `GET /projects/{name}/audit-pack` —
and a credential's `projects` list is what it is checked against. An empty list
is every project, because that is what a platform scope means when nobody
...
**It is not a person, and the stakes are higher than for a CI key.** An
installation that has never named its operators seeds the list from the accounts
that exist ([Bootstrap](#bootstrap-and-what-happens-on-upgrade)), so an account
counted as a person here would be a credential that becomes an operator by
upgrade. The reserved domain is what stops that, exactly as it does for a
machine account, and `POST /projects` refuses a platform credential for the same
reason it refuses a CI key: a project's creator becomes its admin, and an admin
issues keys.

**Nothing changes at the identity provider.** What a platform credential may do
is Kitchen's own state, so the widening happens entirely on Kitchen's side of
the boundary: at the issuer it exchanges itself for a token and reads its own
session, exactly as a CI key does. That is worth stating rather than leaving as
the absence of a case — the tempting version of this feature, one credential
that is simply "more privileged", would have made a key that reaches more of the
platform also reach more of the issuer.

**Where the first one comes from.** `POST /platform/credentials` needs the
operator role, and the only credential a person can hold in a CLI is one of
these — so a fresh installation still reaches the screen through a browser once.
That is the [cluster-bootstrap exception](../CLAUDE.md) the install, the DNS and
the bootstrap link already live in, rather than a new one, and it is narrower
than what it replaced: the browser is needed once for the platform rather than
every time a platform command is run.

See [docs/api/platform.md](api/platform.md) for the routes and
...
| A credential that reaches the platform | A machine account holding **scopes** on the singleton, with a required expiry | A fourth platform role would have to be ordered against `operator`, which is a question with no honest answer. A scope is an operation; a route names one or is the operator's forever, and none of the routes that issue credentials, change the operator list, upgrade the platform or write a connection ever will |
...
to where a string is kept. That is issue #370; this is what holds until it
lands. (It was to have been designed alongside #349, which has since landed
without touching what a browser session is: a platform credential is a
machine's, and the question of where a *person's* refresh token lives is
untouched by it.)
...
credential — something the platform holds on somebody's behalf, as a
connection's token and a project's own secret are, and as a literal a
developer typed into an environment variable is not (see [what each surface
requires](#what-each-surface-requires)) — and the platform's rule is that the
API never reads a credential back. Routing a password change through the
operator would put every password on the installation through a service that has no reason to see one, in order
