...
5. **Log pipeline.** ✅ Shipped for logs: a Vector DaemonSet tails every container on the node and ships the lines into ClickHouse, with `project`, `environment` and `build` lifted out of the Kitchen labels into columns — and a JSON line's own fields flattened into a `fields` map, so `http.status:500` is a column lookup rather than a substring search. The operator owns the schema and its TTL (`Kitchen.spec.observability.clickhouse.retentionDays`). The operator API reads them back per build and per environment, with a window, a limit and a substring search. On top of that sits log *analytics* rather than a log grep: one selection — Kitchen's query language, or raw ClickHouse as the escape hatch — asked four ways, as lines, as a histogram over the window, as facets with counts, and as message patterns. Deploy events and Hubble flow data reuse the same store and are collected too — `internal/flows` streams the flows in, the traffic view draws the service map from them, and the overview's numbers are derived from all three signals. Resource metrics and traces are the half that is missing ([#73](https://github.com/Bermos/Kitchen/issues/73)), and with traces goes the correlation half (click a slow request, get its trace, get the logs of the pods it touched).
