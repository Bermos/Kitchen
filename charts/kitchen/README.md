...
| `kitchen.observability.signals.policy.preset` | `balanced` | What this installation counts as worth hearing, as one of three named presets: `strict`, `balanced` or `homelab`. `balanced` is the platform's own judgement and is the compiled-in numbers exactly, so an installation that leaves this alone behaves as it always did; `homelab` is one host and a handful of projects, where two projects failing together is a correlation and nothing pages at all. A seed rather than the source of truth — the preset and the six numbers underneath it are edited afterwards on `/platform/policy`. |
